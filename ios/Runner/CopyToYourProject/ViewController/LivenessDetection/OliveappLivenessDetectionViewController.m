...
#import "OliveappCameraConfig.h"
#import "OliveappCameraVideoSaveDelegate.h"
...
// 活检是否成功
@property BOOL mIsLivenessSucc;
// 是否开启录制视频
@property BOOL mEnableRecording;
...
- (void)viewDidLoad {
    ...
    /**
     * 下面的代码用于debug模式时使用，可以删除
     */
    ShareData * shareData = [ShareData sharedInstance];
    if ([shareData.model isEqualToString:@"debug"]) {
        if (!shareData.withPrestart) {
            _mVerificationControllerType = WITHOUT_PRESTART;
            _mIsInPrestart = NO;
        }
    }
    /**上面代码用于debug模式时使用，可以删除**/
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    ShareData * shareData = [ShareData sharedInstance];
    self.mEnableRecording = shareData.enableRecording;
    
    OliveappCameraConfig *config = [[OliveappCameraConfig alloc] init];
    config.facing = AVCaptureDevicePositionFront;
    config.recordingEnabled = self.mEnableRecording;
    config.videoDirectory = [OliveappFileManager getDocumentsPath];
...
//                         withResolution:AVCaptureSessionPresetPhoto];
        config.resolution = AVCaptureSessionPresetPhoto;
    } else {
//        [_mCameraController startCamera:AVCaptureDevicePositionFront
//                         withResolution:AVCaptureSessionPresetHigh];
        config.resolution = AVCaptureSessionPresetHigh;
    }
    
    [self.mCameraController startCamera:config];
    [self.mCameraController setCameraSaveVideoDelegate:self];
    ...
    // 如果跳过预检，并且启用了录制视频功能，那么就在这里开始录制。
    // 如果带预检，那么录制视频应该放到预检通过之后开始，也就是在onPrestartSuccess回调里。
    if (self.mVerificationControllerType == WITHOUT_PRESTART && self.mEnableRecording) {
        if (![self.mCameraController startRecording]) {
            NSLog(@"开启录制视频失败");
        }
    }
}
...
- (void) startDetection:(NSError **) error {
    ...
    [config usePredefinedConfig: 7]; // 使用默认的配置
    
    /**
      * 下面的代码用于搭配debug模式时使用，集成时请删除
      */
    ShareData * shareData = [ShareData sharedInstance];
    if ([shareData.model isEqualToString:@"debug"]) {
        NSLog(@"进入debug模式");
        config.totalActions = shareData.actionCount;
        config.minPass = config.totalActions;
        config.timeoutMs = shareData.actionTime;
        if (shareData.fixActionList) {
            config.actionList = [NSMutableArray arrayWithArray:shareData.actionListArray];
        }
        config.fanpaiClsImageNumber = shareData.fanpaiClsImageNumber;
        config.saveRgb = shareData.saveRgb;
        config.saveOriginImage = shareData.saveOriginImage;
        config.savePackage = shareData.savePackage;
        config.saveFanpaiCls = shareData.saveFanpaiCls;
        config.saveJPEG = shareData.saveJPEG;
        // 昏暗检测阈值
        config.darkLevel = shareData.darkLevel;
        // 是否传场景图
        config.newPackage = shareData.newPackage;
    }
    /** 上面的代码用于搭配debug模式时使用，集成时请删除 */
     ...
     * 这个字段用于防重放攻击
     */
    config.userDefinedContent = @"ios_user_defined_content";
...
- (void) onLivenessSuccess: (OliveappDetectedFrame*) detectedFrame
              withFaceInfo:(OliveappFaceInfo *)faceInfo{
    self.mIsLivenessSucc = YES;
    
    if (self.mEnableRecording) {
        [self.mCameraController stopRecording];
    }
    ...
    NSLog(@"[END] LivenessDetectionViewController::onLivenessSuccess");
        NSString *base64Str = [detectedFrame.verificationData base64EncodedStringWithOptions:0];
        _result([NSString stringWithFormat:@"%@",base64Str]);
}
...
/**
 录制视频的回调函数。
 如果活检成功，则拷贝视频到相册（需要申请权限）；否则，则删除视频。

 @param url 视频路径
 */
- (void) onSaveVideoSuccess: (NSURL *) url {
    NSLog(@"保存视频回调：%@。", url);
    if (self.mIsLivenessSucc) {
        NSLog(@"活检通过，拷贝视频到Camera Roll");
        [OliveappFileManager copyVideoToCameraRoll:url];
    } else {
        NSLog(@"活检未通过，删除视频");
        [OliveappFileManager removeFile:url];
    }
}
